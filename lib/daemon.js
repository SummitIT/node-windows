...
Object.defineProperties(this,{
 ...
 directory: {
   ...
   value: function(dir){
     ...
     let instanceDaemonDir = path.resolve(path.join(this._directory,daemonDir));
     if(config.daemonDir!=null) {
       let _tempDir =  config.daemonDir;
       console.log('instanceDaemonDir|_tempDir',_tempDir);
       instanceDaemonDir = path.resolve(path.join(_tempDir,daemonDir));
     }
     if (dir){
      instanceDaemonDir = path.resolve(path.join(path.resolve(dir),daemonDir));
     }
     console.log('instanceDaemonDir',instanceDaemonDir);
     return instanceDaemonDir;
   }
 },
 ...
 execPath: {
  ...
  // Update the xml configuration file and save back to the computer.
  updateXML: {
    enumerable: true,
    writable: false,
    configurable: false,
    value: function(dir) {
      console.log('updateXML called');
     if (typeof dir === 'function'){
       callback = dir;
       dir = null;
     }
     dir = this.directory(dir);
     let fileName = path.resolve(dir,this.id+'.xml');
     let me = this;
     fs.writeFile(fileName,this._xml,function(err){
       if(err) {
         console.log('fs.writeFile|XML|error',err);
         me.emit('error',err);
       } else {
     ...
     fs.exists(dir,function(exists){
       if (!exists){
         try {
           ...
           if(err.message.indexOf('EPERM: operation not permitted, mkdir')!=-1) {
             me.emit('error','Permissions Error: Unable to create folder.');
           } else {
             me.emit('error',err);
           }
           return;
       ...
       fs.writeFile(path.resolve(dir,me.id+'.xml'),me._xml,function(err){
         if(err) {
           ...
           winsw.createExe(me.id,dir,function(err){
             if(err) {
               console.log('winsw.createExe|error',err);
               me.emit('error',err);
             } else {
               me.execute('"'+path.resolve(dir,me._exe)+'" install',function(err){
                 if(err) {
